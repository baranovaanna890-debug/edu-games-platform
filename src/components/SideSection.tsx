...
export function StatsSection({ completedGames, totalXP }: StatsProps) {
  return (
    <div>
      <h2 className="font-game text-2xl text-purple-700 mb-1">
        📊 Мой прогресс
      </h2>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white rounded-xl border-2 border-purple-100 p-4 text-center">
          ...
          <div className="font-game text-2xl text-purple-700">
      ...
      {completedGames.length === 0 ? (
        <div className="bg-white rounded-xl border-2 border-gray-100 p-8 text-center">
          ...
          <p className="text-gray-500 text-sm">
            Ты ещё не прошёл ни одной игры.
            <br />
            Перейди в каталог и начни!
          </p>
        </div>
      ) : (
            ...
            {completedGames.map((id) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-xs font-bold px-2 py-0.5 rounded-full ${difficultyClass[g.difficulty]}`}
                  >
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto">
      <div className="mb-8 text-center">
        <h2 className="font-game text-3xl text-purple-700 mb-2">
          🏆 Сертификат
        </h2>
        <p className="text-gray-500">
          Введи имя и фамилию для создания сертификата
        </p>
      </div>
      ...
      <div className="card-game rounded-2xl p-6 mb-6">
        <div className="space-y-4">
          ...
          <button
            onClick={() => {
              if (certName && certSurname) setCertShown(true);
            }}
      ...
      {certShown && certName && certSurname && (
        <div className="animate-scale-in">
          <div className="certificate-bg rounded-2xl p-8 text-center relative overflow-hidden">
            ...
            <div className="absolute bottom-3 right-3 text-2xl opacity-60">
              ⭐
            </div>
            ...
            <h2 className="font-game text-3xl text-purple-800 mb-1">
              КодоГерой
            </h2>
            ...
            <h3 className="font-game text-2xl md:text-3xl text-purple-700 mb-2">
              {certName} {certSurname}
            </h3>
            <p className="text-gray-600 text-sm mb-6">
              успешно прошёл(а) курс дидактических игр по информатике для 7-9
              классов
            </p>
            ...
            <div className="certificate-seal w-16 h-16 rounded-full mx-auto flex items-center justify-center text-2xl mb-4">
              🏆
            </div>
          ...
          <div className="mt-4 text-center">
            <p className="text-sm text-gray-500">
              Распечатай сертификат или сделай скриншот! 📸
            </p>
          </div>
        </div>
      )}
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto">
          ...
          (player, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="font-game text-xl mb-1">
                  {pos === 1
                    ? palette[1].medal
                    : pos === 2
                      ? palette[0].medal
                ...
                <div className="font-semibold text-xs text-gray-700 text-center">
                  {player.name}
                </div>
                <div className="text-xs text-gray-400 mb-1">{player.grade}</div>
                <div className="font-game text-xs text-yellow-500">
                  ⚡ {player.xp}
                </div>
              </div>
            );
      ...
      <div className="card-game rounded-2xl overflow-hidden">
        {mockLeaderboard.map((player, idx) => (
          <div
            ...
            style={{
              borderBottom:
                idx < mockLeaderboard.length - 1 ? "1px solid #f3f4f6" : "none",
            }}
          >
            <div
              className="w-8 text-center font-game text-lg"
              style={{
                color:
                  idx === 0
                    ? "#f59e0b"
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-gray-400">
                {player.grade} · {player.games} игр
              </div>
            </div>
            <div className="text-right">
              <div className="font-game text-sm text-yellow-500">
                ⚡ {player.xp} XP
              </div>
              <div className="text-xs text-yellow-400 mt-0.5">
                {"⭐".repeat(Math.min(Math.floor(player.xp / 1000), 5))}
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="mt-4 card-game rounded-xl p-4 text-center border-2 border-purple-200 bg-purple-50">
        ...
        <p className="font-game text-lg text-purple-700">
          ⚡ {totalXP} XP · {completedGames.length} игр пройдено
        </p>
      </div>
    </div>
  );
